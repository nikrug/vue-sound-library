...
interface Address {
  ...
  entrance: string;
  apartment2: string;
  showDeletePopup?: boolean; // Новое свойство для контроля попапа удаления
}
  ...
  setup() {
    ...
    const handleAddAddress = (newAddress: Address) => {
      if (addresses.value.length < 4) {
        addresses.value.push({ ...newAddress, showDeletePopup: false }); // Инициализируем свойство
        saveAddresses();
      }
    ...
    const removeAddress = (index: number) => {
      ...
      saveAddresses();
      // Закрываем попап после удаления
      addresses.value[index].showDeletePopup = false; // уберите это, если не требуется
    };
